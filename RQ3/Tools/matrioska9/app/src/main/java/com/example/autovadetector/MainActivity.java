package com.example.autovadetector;
...
public class MainActivity extends AppCompatActivity implements VirtualCore.InstallCallback {

    public static final String TAG = "VADetector";

    private DataCollector data;

    private InstallResult install_result;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);
        analyze("/sdcard/"+getSample(new File("/sdcard/")));
    }

    private static boolean explore(File file) {
        if (file.isDirectory()) {
            Log.d(TAG, file.getAbsolutePath()+" is directory");
            ...
            if(file.getName().contains("VADetector")) {
                return true;
            }
        }
        return false;
    ...
    public void analyze(String path) {
        Log.d(TAG, "Plugin: "+path);
        install_result = null;
        install_plugin(path);
        while(true) {
            if(install_result != null) break;
            try {
                Thread.sleep(500);
            } catch (Exception e) {}
        }
        if (install_result.isSuccess) {
            ...
            run_plugin(install_result.packageName);
            Handler handler = new Handler();
            handler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    data.addProcessesSample(nProcesses());
                    File[] apks = searchApk(new File("/data/user/0/com.example.autovadetector/virtual/data/user/0/"+install_result.packageName+"/files"));
                    for(int i = 0; i < apks.length; i++)
                        data.addFoundApk(apks[i]);
                    apks = searchApk(new File("/data/user/0/com.example.autovadetector/virtual/storage/emulated/0"));
                    for(int i = 0; i < apks.length; i++)
                        data.addFoundApk(apks[i]);
                    uninstall_plugin(install_result.packageName);
                    data.readAll();
                    verdict(install_result.packageName, path);
                    data = null;
                    install_result = null;
            ...
            Log.e(TAG, "Error: Cannot install plugin! - "+install_result.error);
            Log.d(TAG, "COMPLETED! "+path);
            data = null;
            install_result = null;
            finish();
    ...
    private String verdict(String packageName, String apkpath) {
        ...
        // Delete APKs
        deleteDir(new File(DataCollector.FOLDERPATH_READ));
        // Intent Analysis
        ...
        for(int i = 0; i < intents.length; i++) {
            ...
            if((!intents[i][2].equals("null")) && (!intents[i][2].equals(packageName)) && (!intents[i][2].equals("com.example.autovadetector"))) {
        ...
        Log.d(TAG, str);
        for(int i = 0; i < uSize; i++) {
            Log.d(TAG, "Unknown Intent: "+unknown[i]);
        }
        return str;
    }
...
//    }
    ...
    private void install_plugin(String path) {
        VirtualCore.get().installPackage(path, InstallOptions.makeOptions(false), this);
    }
