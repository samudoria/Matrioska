...
public class DataCollector {
    ...
    //private static final String PATH = Environment.getExternalStorageDirectory().getPath()+"/VADetector_tmp.txt";
    private static final String PATH_WRITE = "/sdcard/VADetector_tmp.txt";
    private static final String PATH_READ = "/data/user/0/com.example.autovadetector/virtual/storage/emulated/0/VADetector_tmp.txt";
    public static final String FOLDERPATH_WRITE = "/sdcard/VAD_APKS";
    public static final String FOLDERPATH_READ = "/data/user/0/com.example.autovadetector/virtual/storage/emulated/0/VAD_APKS";
        ...
        try {
            ...
            while (s.hasNextLine()) {
                ...
                if(line.startsWith("Application: ")) {
                    ...
                    if((!value.equals("com.example.autovadetector")) && (!MainActivity.isIn(value, applications, aSize))) {
            ...
            Log.d("VADetector_Collector", "read error "+e.toString());
            e.printStackTrace();
        }
        data.delete();
    }
    ...
    public static void addPermissions(int data) {
        try {
            Writer output = new BufferedWriter(new FileWriter(PATH_WRITE, true));
    ...
    public static void addFoundApk(File file) {
        File dir = new File(FOLDERPATH_WRITE);
        if(!dir.exists()) dir.mkdir();
        File dst = new File(FOLDERPATH_WRITE+"/"+file.getName());
        int i = 1;
        while(dst.exists()) {
            dst = new File(FOLDERPATH_WRITE+"/"+i+file.getName());
            i++;
        }
        try {
            Writer output = new BufferedWriter(new FileWriter(PATH_WRITE, true));
            String[] n = dst.getAbsolutePath().split("/");
            output.append("FoundApk: "+FOLDERPATH_READ+"/"+n[n.length-1]+"\n");
            output.close();
