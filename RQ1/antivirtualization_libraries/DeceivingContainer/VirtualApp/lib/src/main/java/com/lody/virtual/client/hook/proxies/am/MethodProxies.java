...
import com.lody.virtual.client.hook.utils.BluetoothOppUtils;
...
import dev.sime1.patch.IntentSanitizer;
...
//            if(!BluetoothOppUtils.checkIntent(intent)) {
//                return 0;
//            }

            intent = VActivityManager.get().sanitizeIntent(intent);
            if(intent == null) {
                Log.e("sanitize-intent","intent has been blocked");
                return 0;
            }
            ...
            if (resultTo != null) {
                ...
                ResultSanitizer.addLaunched(intent, requestCode);
            }
            int userId = VUserHandle.myUserId();
...
//            }
            ...
            if (activityInfo == null) {
                ...
                VLog.d("VActivityManager", "resolveInfo: %s", resolveInfo);
                ...
                // make guest apps able to call quick contact activity
                if(ContactsContract.QuickContact.ACTION_QUICK_CONTACT.equals(intent.getAction())) {
                    Uri data = intent.getData();
                    String id = data.getLastPathSegment();
                    data = Uri.parse(ContactsContract.Contacts.CONTENT_URI + "/" + id);
                    intent.setData(data);
                    return method.invoke(who, args);
                }
                ...
                VLog.d("VActivityManager", "invoke original method %d", args.length);
                Intent i = (Intent) args[intentIndex];
                VLog.d("VActivityManager", "intent: " + i.getData());
                VLog.d("VActivityManager", "activity: " + resolveInfo.activityInfo.name);
                return method.invoke(who, args);
            }
