...
public final class VirtualCore {
    ...
    public void startup(Context context, SettingConfig config) throws Throwable {
        if (!isStartUp) {
            ...
            // IntentSanitizer.addSanitizer(filterBTIntent);
            // NativeEngine.dynamicHooks(getContext().getCacheDir(), "nopackages.apk", "dev.sime1.patch.Patch");

            // TODO: this seems to be called multiple times, so we should try to find a different
            // place to use to preload the patches, otherwise there is not benefit. We could also
            // just install them in cache once.
