...
public final class BinderProvider extends ContentProvider {
    ...
    private boolean init() {
        ...
        // we need to apply dynamic hooks here as well, so that if the "bug" crosses process
        // boundaries (e.g. it involves a call to a service) we fix it in the server process as
        // well
        // TODO: remove generic dynamic hooks and only load server hooks

        Trace.beginSection("ServerPatch");
        NativeEngine.applyServerPatch(context.getCacheDir());
        Trace.endSection();
        // NativeEngine.dynamicHooks(context.getCacheDir());
        // TODO: cleanup
        if (context != null) {
